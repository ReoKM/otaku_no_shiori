...
### F7. 共有画像生成(ファーストリリースには入れない → Phase 2)

> **2026-07-29オーナー決定: F7はファーストリリースに含めず Phase 2 に送る。**
> 実装は完了しており(S5画面・生成Function・テンプレート2種)、`src/lib/feature-flags.ts` の
> `FEATURE_SHARE` を `false` にして導線だけ閉じている。コードは削除していないため、
> Phase 2 でフラグを `true` に戻せば下記の仕様どおり動く。
> 以下はその際に有効な仕様として残す。
...
- ログインを促すタイミング: しおり保存後のバナー表示、共有画像生成後の一言、の2箇所のみ。しつこくしない(共有画像がPhase 2送りのため、ファーストリリースでは前者の1箇所のみ)
...
- ハンバーガーメニュー(S1/S3ヘッダー)から「アプリをインストール」を明示的に呼び出せる(対応ブラウザは`beforeinstallprompt`でネイティブダイアログ、iOS Safariは手動追加の案内文。オーナー指示により追加、2026-08-23)
...
| S5 | 共有画像プレビュー | テンプレ選択→プレビュー→画像保存 ※F7と同じくPhase 2送り。導線は閉じており、直リンク時は記録タブへ戻す |
...
flowchart TD
    ...
    S4 -->|しおりに追加| S3
    S3 -.->|共有画像を作る Phase 2| S5[S5 共有画像プレビュー]
    S5 -.->|画像を保存| S3
    S1 --> S6[S6 設定/アカウント]
    S5 -.->|ログイン促し| S6
```
...
**ファーストリリースでは1のみ。** 2〜3はF7がPhase 2送りのため、そのタイミングで有効になる。
...
- 共有画像生成(F7・S5) → Phase 2 ※2026-07-29オーナー決定。実装済みだが`FEATURE_SHARE`で導線を閉じている(コードは残す)
