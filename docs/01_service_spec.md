...
- 作成後は「しおり詳細」画面がホームになり、タブで各機能(持ち物/TODO/旅程/スポット/ログ)を切り替える
- しおりは複数作成できる(一覧画面あり)
...
- 場所名にはGoogleマップ検索リンクを自動付与(APIキー不要の `https://www.google.com/maps/search/?api=1&query=場所名` 形式)
- エントリーの追加・編集・削除・並べ替え
...
- MVPでは1しおりあたり写真20枚まで(ストレージ無料枠保護のため。上限は設定で変更可能な実装にする)
...
- **ゲスト利用**: 未ログインでも全機能が使える。データは端末(localStorage/IndexedDB)に保存
...
| S3 | しおり詳細 | タブ切替: 持ち物(S3a)/TODO(S3b)/旅程(S3c)/スポット(S3d)/ログ(S3e) |
