...
Netlify無料枠の天井(クレジット制): **月間300クレジットのハードリミット**。超過すると全プロジェクトが一時停止し(「Site not available」表示)、無料プランは自動リチャージ不可のため月次リセットまで復旧できない。主な消費レートは本番デプロイ15クレジット/回・帯域20クレジット/GB。

このため次の運用ルールを必ず守る:

1. **mainへのマージごとに本番デプロイしない**。auto-publishを無効化(Locked deploys)し、本番反映は週1〜2回のまとめデプロイにする(毎マージ公開だと20回/月で枠が尽きる)
2. PRの動作確認はプレビューデプロイで行う(無料枠でプレビューは無制限)
3. 重い写真データはSupabase Storage側に載せ、Netlify側の帯域消費を抑える

クレジット超過が見えたらNetlify Pro($20/月、3,000クレジット)へのアップグレードかCloudflare(有料)への移行をオーナーが判断する。
...
| 項目 | MVP期 | 成長期の目安 |
|------|-------|--------------|
| Netlify | 0円 | Pro $20/月 or Cloudflare(有料)へ移行を判断 |
